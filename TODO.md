...
| 56 | Uji perangkat APK `app-preview`: tampilan edge-to-edge, layanan VPN tidak ditolak sistem (targetSdk 36), bilah kembali, dan tawaran kesiapan (Always-on VPN + baterai). Verifikasi: `adb shell settings get secure always_on_vpn_app`/`_lockdown`, lalu `adb shell am force-stop com.rollinkxx.velum.preview` | Tinggi | Belum — butuh maintainer (tidak bisa diuji dari sandbox) |
...
| 61 | Tawaran sekali: Always-on VPN + blokir tanpa VPN & bebas optimasi baterai, keputusan diuji unit (`VelumSetup.offer`) | Tinggi | Selesai tervalidasi (CI run 34684549219) — perilaku pengaturan sistem tetap perlu uji perangkat |
