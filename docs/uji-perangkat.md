...
| H6 | V1 | Buka aplikasi; lihat judul "Velum" dan tagline tepat di bawahnya. | Judul **lebih besar dari sebelumnya** dan membentang hampir selebar kartu status (menyentuh batas kiri/kanan area isi, bukan hanya di tengah), tetap **satu baris**, dan **seluruh huruf "Velum" terlihat utuh — tanpa titik-titik (…), tanpa huruf terpotong, tanpa huruf hilang**. Tagline menempel dekat di bawah judul (jarak terlihat ±14dp, jauh lebih rapat dari sebelumnya yang ±29dp) — tetapi **tidak** menempel/menabrak huruf judul | Judul tampil `Vel…` / ada titik-titik setelah "Vel" = `singleLine` dipasang lagi atau auto-size tidak menyusut (lihat komentar jebakan di `activity_main.xml`); judul tetap kecil/selebar sebelumnya = batas auto-size tidak berubah; judul membungkus dua baris / terpotong = lebar mengikat tapi `maxLines`/auto-size gagal; tagline menabrak judul = margin negatif terlalu besar untuk skala huruf perangkat ini (laporkan angkanya, jangan perbaiki sendiri) |
| H7 | V1 | Dalam keadaan tersambung dan **tanpa pesan apa pun**, lihat kartu status: perhatikan jarak antara badge "Tersambung" dan garis pemisah di bawahnya, lalu ukur tinggi kartu secara kasar terhadap tombol "Putuskan" (52dp). Lalu **pancing** munculnya pesan: tekan "Uji koneksi" saat tunnel belum tersambung, atau cabut jaringan. | Tanpa pesan: tidak ada celah kosong di bawah "Tersambung" — garis pemisah tepat di bawah badge, kartu terlihat seimbang (tinggi ±144dp, kira-kira 3× tinggi tombol). Saat pesan muncul: barisnya tampil utuh dan kartu ikut memanjang, tanpa menggeser isi layar keluar batas | Celah kosong tetap ada saat tanpa pesan = `visibility` tidak diterapkan; pesan tidak pernah muncul = terlalu agresif menyembunyikan (lapor); kartu memanjang sampai ada isi terpotong = pesan panjang perlu pemeriksaan `maxLines` |
